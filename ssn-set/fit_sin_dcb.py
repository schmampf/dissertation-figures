...
# The symmetric high-bias hump is not part of the single-particle SIN model.
# Keep it visible in the diagnostic plot, but give it zero weight in the fit.
HUMP_ABS_V_RANGE_MV = (0.48, 0.65)
TAIL_ABS_V_MIN_MV = 0.60
plot_mask = np.isfinite(Vbias0_mV) & np.isfinite(Iexp0_nA) & (np.abs(Vbias0_mV) <= 0.65)
hump_mask = (np.abs(Vbias0_mV) >= HUMP_ABS_V_RANGE_MV[0]) & (
    np.abs(Vbias0_mV) <= HUMP_ABS_V_RANGE_MV[1]
)
fit_mask = plot_mask & ~hump_mask
Vplot_mV = Vbias0_mV[plot_mask]
Iplot_nA = Iexp0_nA[plot_mask]
...
Imodel_fit_nA = _sin_fit_model(fit_result.x, Vfit_mV)
Imodel_plot_nA = _sin_fit_model(fit_result.x, Vplot_mV)
fit_rmse_nA = float(np.sqrt(np.mean((Imodel_fit_nA - Ifit_nA) ** 2)))
...
# Second iteration: retain the SIN-specific lineshape found without the hump,
# but redetermine only the normal conductance from dI/dV on the high-bias
# tails. Differentiation removes the accumulated current displacement caused
# by the hump and directly estimates the asymptotic normal conductance.
voltage_step_mV = float(np.median(np.diff(Vplot_mV)))
...
unit_model_nA = simulate_sin_current_nA(
    Vplot_mV,
    G_T_uS=1.0,
    T_K=T_K,
    Delta_meV=Delta_meV,
...
unit_dIdV = np.gradient(unit_model_nA, Vplot_mV)
tail_plot_mask = np.abs(Vplot_mV) >= TAIL_ABS_V_MIN_MV
tail_data_uS = dIdV_exp_uS[tail_plot_mask]
tail_unit_dIdV = unit_dIdV[tail_plot_mask]
G_T_tail_uS = float(
    np.dot(tail_unit_dIdV, tail_data_uS) / np.dot(tail_unit_dIdV, tail_unit_dIdV)
)
tail_residual_uS = tail_data_uS - G_T_tail_uS * tail_unit_dIdV
G_T_tail_std_uS = float(
    np.sqrt(
        np.dot(tail_residual_uS, tail_residual_uS)
        / max(1, tail_residual_uS.size - 1)
        / np.dot(tail_unit_dIdV, tail_unit_dIdV)
    )
)
G_T_tail_G0 = G_T_tail_uS / sc.G0_muS
G_T_tail_G0_std = G_T_tail_std_uS / sc.G0_muS
R_T_tail_kOhm = 1e3 / G_T_tail_uS
R_T_tail_std_kOhm = R_T_tail_kOhm * G_T_tail_std_uS / G_T_tail_uS
Imodel_tail_G_nA = simulate_sin_current_nA(
    Vplot_mV,
    G_T_uS=G_T_tail_uS,
    T_K=T_K,
    Delta_meV=Delta_meV,
...
tail_rmse_uS = float(np.sqrt(np.mean(tail_residual_uS**2)))
...
print(
    ...
    f"Second iteration, |V| >= {TAIL_ABS_V_MIN_MV:g} mV; "
    "SIN shape fixed:\n"
    f"G_T_tail_uS   = {G_T_tail_uS:.6g} +/- {G_T_tail_std_uS:.2g}\n"
    f"G_T_tail_G0   = {G_T_tail_G0:.6g} +/- {G_T_tail_G0_std:.2g}\n"
    f"R_T_tail_kOhm = {R_T_tail_kOhm:.6g} +/- {R_T_tail_std_kOhm:.2g}\n"
    f"tail_RMSE_uS  = {tail_rmse_uS:.6g}"
)

dIdV_model_uS = np.gradient(Imodel_plot_nA, Vplot_mV)

fig, axes = plt.subplots(2, 1, figsize=(5.0, 5.5), sharex=True)
axes[0].plot(Vplot_mV, Iplot_nA, color="0.55", lw=0.8, label="data")
axes[0].plot(Vplot_mV, Imodel_plot_nA, color="C3", lw=1.2, label="SIN fit")
axes[0].plot(
    Vplot_mV,
    Imodel_tail_G_nA,
    color="C0",
    lw=1.0,
    ls="--",
    label="tail-fitted $G_T$",
)
axes[0].set_ylabel(r"$I$ (nA)")
axes[0].legend(frameon=False)
axes[1].plot(Vplot_mV, dIdV_exp_uS, color="0.55", lw=0.8)
axes[1].plot(Vplot_mV, dIdV_model_uS, color="C3", lw=1.2)
axes[1].plot(
    Vplot_mV,
    np.gradient(Imodel_tail_G_nA, Vplot_mV),
    color="C0",
    lw=1.0,
    ls="--",
)
...
for ax in axes:
    for sign in (-1.0, 1.0):
        bounds = np.sort(sign * np.asarray(HUMP_ABS_V_RANGE_MV))
        ax.axvspan(
            bounds[0],
            bounds[1],
            color="C0",
            alpha=0.08,
            lw=0.0,
            label="excluded hump" if ax is axes[0] and sign > 0 else None,
        )
axes[0].legend(frameon=False)
fig.tight_layout()
fit_figure_path = Path(__file__).with_name("fit_sin_dcb.png")
